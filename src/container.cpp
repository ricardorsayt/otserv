...
Container::Container(uint16_t type, uint16_t size, bool unlocked, bool pagination) :
	Item(type),
	maxSize(size),
	unlocked(unlocked),
...
uint32_t Container::getWeight() const 
{
	uint32_t baseWeight = Item::getWeight();
	uint32_t contentWeight = totalWeight;
	uint32_t fullWeight = baseWeight + contentWeight;
	
	static const std::unordered_set<uint16_t> reducedWeightContainerIDs = {
		5569,
		5545,
		5546,
	};

	if (reducedWeightContainerIDs.find(getID()) != reducedWeightContainerIDs.end()) {
		double reductionFactor = 0.8;
		fullWeight = baseWeight + static_cast<uint32_t>(contentWeight * reductionFactor);
	}

	return fullWeight;
}

std::string Container::getContentDescription(bool includeItemID) const
{
	std::ostringstream os;
	return getContentDescription(os, includeItemID).str();
}

std::ostringstream& Container::getContentDescription(std::ostringstream& os, bool includeItemID) const
{
	bool firstitem = true;
		...
		uint32_t flags, Creature* actor/* = nullptr*/) const
{
	...
	if (item->isStoreItem() && !dynamic_cast<const DepotLocker*>(this)) {
	...
	if (item->isStoreItem() && dynamic_cast<const StoreInbox*>(cylinder)) {
		ReturnValue ret = RETURNVALUE_ITEMCANNOTBEMOVEDTHERE;
		if (!item->isStoreItem()) {
	...
	if (getWeaponType() == WEAPON_QUIVER) {
		if (item->getWeaponType() != WEAPON_AMMO || (getAmmoType() != item->getAmmoType())) {
			if (getAmmoType() == AMMO_BOLT) {
				return RETURNVALUE_QUIVERBOLTAMMOONLY;
			}
			else if (getAmmoType() == AMMO_ARROW) {
				return RETURNVALUE_QUIVERARROWAMMOONLY;
			}
		}
		
	} else if (getWeaponType() == WEAPON_POUCH)
	{
		if (getAmmoType() == AMMO_STONE && item->getShootType() != CONST_ANI_SMALLSTONE) {
			return RETURNVALUE_CANNOTMOVEITEMINSIDESMALLSTONEPOUCH;
		}
	}
