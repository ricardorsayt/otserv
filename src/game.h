...
class Game
{
	public:
		...
		ReturnValue internalRemoveItem(Item* item, int32_t count = -1, bool test = false, uint32_t flags = 0, bool ignoreEvent = false);
		...
		std::vector<Item*> findPouches(Cylinder* cylinder, uint16_t itemId, bool depthSearch = true, int32_t subType = -1) const;
		...
		void playerToggleOutfitExtension(uint32_t playerId, int wings, int aura, int shader);
		...
		void updateMonsterStar(Monster* monster);
		...
		bool playerCloseBattlepass(uint32_t playerId);
		bool playerModifyQuest(uint32_t playerId, uint8_t id, uint16_t questId);
		bool playerBuyPremiumBattlepass(uint32_t playerId);
		bool isBattlepassActive() const { return battlepassActive; }
		...
		void addAnimatedText(const Position& pos, TextColor_t textColor, const std::string& text, bool critical = false);
		static void addAnimatedText(const SpectatorVec& spectators, const Position& pos, TextColor_t textColor, const std::string& text, bool critical = false);
	...
	private:
		...
		std::mutex accountLoginAttemptsLock;
		std::mutex ipLoginAttemptsLock;
