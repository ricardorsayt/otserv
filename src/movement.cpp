...
static std::set<uint32_t> mageVocationIds{
	1, 2, 5, 6
};
...
ReturnValue MoveEvent::EquipItem(MoveEvent* moveEvent, Player* player, Item* item, slots_t slot, bool isCheck)
{
	...
	item->updateItemAttributes(player);
	...
	uint32_t healthRegen = item->getIntCustomAttribute(player, std::to_string(ITEM_RND_HEALTH_REGEN));
	uint32_t manaRegen = item->getIntCustomAttribute(player, std::to_string(ITEM_RND_MANA_REGEN));

	if (healthRegen > 0 || manaRegen > 0)
		player->updateRegeneration();

	bool rngSkills = false;

	if (item->getIntCustomAttribute(player, std::to_string(ITEM_RND_SKILL)) > 0) {
		switch (it.weaponType) {
			case WEAPON_DISTANCE:
			case WEAPON_QUIVER:
				player->setVarSkill(SKILL_DISTANCE, item->getIntCustomAttribute(player, std::to_string(ITEM_RND_SKILL)));
				rngSkills = true;
				break;
			case WEAPON_SHIELD:
				player->setVarSkill(SKILL_SHIELD, item->getIntCustomAttribute(player, std::to_string(ITEM_RND_SKILL)));
				rngSkills = true;
				break;
			case WEAPON_AXE:
				player->setVarSkill(SKILL_AXE, item->getIntCustomAttribute(player, std::to_string(ITEM_RND_SKILL)));
				rngSkills = true;
				break;
			case WEAPON_SWORD:
				player->setVarSkill(SKILL_SWORD, item->getIntCustomAttribute(player, std::to_string(ITEM_RND_SKILL)));
				rngSkills = true;
				break;
			case WEAPON_CLUB:
				player->setVarSkill(SKILL_CLUB, item->getIntCustomAttribute(player, std::to_string(ITEM_RND_SKILL)));
				rngSkills = true;
				break;
			default:
				break;
	...
	std::map<int32_t, int32_t> statAttributes;

/*	if (item->getIntCustomAttribute(player, std::to_string(ITEM_RND_MAGIC)) > 0) {
		if (mageVocationIds.find(player->getVocationId()) != mageVocationIds.end()) {
			statAttributes[CONDITION_PARAM_STAT_MAGICPOINTS] = item->getIntCustomAttribute(player, std::to_string(ITEM_RND_MAGIC));
		}
		rngSkills = true;
	}*/

	if (item->getIntCustomAttribute(player, std::to_string(ITEM_RND_MAX_MANA)) > 0) {
		statAttributes[CONDITION_PARAM_STAT_MAXMANAPOINTS] = player->getVocation()->getManaGain() * item->getIntCustomAttribute(player, std::to_string(ITEM_RND_MAX_MANA));
		rngSkills = true;
	}

	if (item->getIntCustomAttribute(player, std::to_string(ITEM_RND_MAX_HEALTH)) > 0) {
		statAttributes[CONDITION_PARAM_STAT_MAXHITPOINTS] = player->getVocation()->getHPGain() * item->getIntCustomAttribute(player, std::to_string(ITEM_RND_MAX_HEALTH));
		rngSkills = true;
	}

	if (!statAttributes.empty()) {
		ConditionAttributes* attr = new ConditionAttributes(static_cast<ConditionId_t>(slot), CONDITION_ATTRIBUTES, -1, true, item->getID());
		for (auto& it : statAttributes) {
			attr->setParam(static_cast<ConditionParam_t>(it.first), it.second);
		}
		player->addCondition(attr);
	}
	...
	uint32_t speedTier = item->getIntCustomAttribute(player, std::to_string(ITEM_RND_SPEED));
	if (speedTier > 0)
	{
		g_game.changeSpeed(player, 5 * speedTier);
	}
	...
	if (it.abilities->regeneration) {
		...
		if (it.abilities->manaTicks != 0) {
			...
			std::cout << "mana ticks: " << mana_ticks << std::endl;
		}
...
/*	if (item->getIntCustomAttribute(player, std::to_string(ITEM_RND_MAGIC)) > 0) {
		if (mageVocationIds.find(player->getVocationId()) != mageVocationIds.end()) {
			player->removeCondition(CONDITION_ATTRIBUTES, static_cast<ConditionId_t>(slot));
		}
		rngSkills = true;
	}*/
