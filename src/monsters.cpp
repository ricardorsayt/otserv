...
bool Monsters::deserializeBestiary(const pugi::xml_node& node, Bestiary_t& bestiary)
{
	pugi::xml_attribute attr;
	if ((attr = node.attribute("id"))) {
		bestiary.id = attr.as_int();
	}
	else {
		std::cout << "[Warning - Monsters::deserializeBestiary] Cant load bestiary id." << std::endl;
		return false;
	}

	if ((attr = node.attribute("class"))) {
		bestiary.creatureClass = attr.as_string();
	}
	else {
		std::cout << "[Warning - Monsters::deserializeBestiary] Cant load bestiary class." << std::endl;
		return false;
	}

	if ((attr = node.attribute("prowess"))) {
		bestiary.prowess = attr.as_int();
	}
	else {
		std::cout << "[Warning - Monsters::deserializeBestiary] Cant load bestiary prowess." << std::endl;
		return false;
	}

	if ((attr = node.attribute("expertise"))) {
		bestiary.expertise = attr.as_int();
	}
	else {
		std::cout << "[Warning - Monsters::deserializeBestiary] Cant load bestiary expertise." << std::endl;
		return false;
	}

	if ((attr = node.attribute("mastery"))) {
		bestiary.mastery = attr.as_int();
	}
	else {
		std::cout << "[Warning - Monsters::deserializeBestiary] Cant load bestiary mastery." << std::endl;
		return false;
	}

	if ((attr = node.attribute("difficulty"))) {
		bestiary.difficulty = attr.as_int();
	}
	else {
		std::cout << "[Warning - Monsters::deserializeBestiary] Cant load bestiary difficulty." << std::endl;
		return false;
	}

	if ((attr = node.attribute("occurrence"))) {
		bestiary.occurrence = attr.as_int();
	}
	else {
		std::cout << "[Warning - Monsters::deserializeBestiary] Cant load bestiary occurrence." << std::endl;
		return false;
	}
...
bool Monsters::deserializeSpell(MonsterType* mType, const pugi::xml_node& node, spellBlock_t& sb, const std::string& description)
{
	...
	if (isScripted) {
		...
		if (tmpName == "melee") {
			...
			combat->setParam(COMBAT_PARAM_DISPEL, CONDITION_PARALYZE);
...
MonsterType* Monsters::loadMonster(const std::string& file, const std::string& monsterName, bool reloading /*= false*/)
{
	...
	if ((attr = monsterNode.attribute("classId"))) {
		mType->info.monsterClass = static_cast<MonsterClass_t>(pugi::cast<uint16_t>(attr.value()));
	}
	...
	if ((node = monsterNode.child("bestiary"))) {
		Bestiary_t bestiary;
		if (deserializeBestiary(node, bestiary)) {
			mType->info.bestiary = std::move(bestiary);
		}
	}
	...
	if ((node = monsterNode.child("look"))) {
		if ((attr = node.attribute("type"))) {
			...
			if ((attr = node.attribute("shader"))) {
				...
				if (!shader) {
					std::cout << "[Warning - Monsters::loadMonster] cannot find shader " << attr.as_string() << " " << file << std::endl;
				}
