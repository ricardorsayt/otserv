...
bool ConditionRegeneration::executeCondition(Creature* creature, int32_t interval)
{
	...
	if (internalHealthTicks >= healthTicks) {
		...
		if (healthGain > 0)
		{
			if (getId() == CONDITIONID_ITEM_ATTR)
			{
				if (Player* player = creature->getPlayer()) 
				{
					Condition* condition = player->getCondition(CONDITION_REGENERATION, CONDITIONID_DEFAULT);

					if (condition) // Adiciona mana APENAS se existir condição
						creature->changeHealth(healthGain);
						// , std::cout << "Adiciona health " << healthGain << " (por causa da raridade)" << std::endl;
			...
			if (Player* player = creature->getPlayer()) {
				if (getId() == CONDITIONID_ITEM_ATTR) {
					...
					if (condition) // Adiciona mana APENAS se existir condição
						...
						// , std::cout << "Adiciona mana " << manaGain << " (por causa da raridade)" << std::endl;
...
bool ConditionLight::setParam(ConditionParam_t param, int32_t value)
{
	...
	switch (param) {
		...
		case CONDITION_PARAM_LIGHT_REDUCE:
			lightReduce = (value != 0);
			return true;
