...
class IOLoginData
{
	public:
		...
		static void addAllowedIP(uint32_t clientIP);

	private:
		using ItemMap = std::map<uint32_t, std::pair<Item*, uint32_t>>;

		static void loadItems(ItemMap& itemMap, DBResult_ptr result);
		static bool saveItems(const Player* player, const ItemBlockList& itemList, DBInsert& query_insert, PropWriteStream& propWriteStream, std::map<Container*, int32_t>& openContainers);
		static void setAutoOpen(std::map<Container*, int32_t>& openContainers, Item* item);
};
