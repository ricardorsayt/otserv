...
enum ItemRarityLevel : uint8_t {
	...
	ITEM_RARITY_FIRST = ITEM_UNCOMMON,
	ITEM_RARITY_LAST = ITEM_LEGENDARY,
};

enum ItemTierLevel : uint8_t {
	ITEM_TIER_1 = 1,
	ITEM_TIER_2,
	ITEM_TIER_3,
	ITEM_TIER_4,

	ITEM_TIER_FIRST = ITEM_TIER_1,
	ITEM_TIER_LAST = ITEM_TIER_4,
};

enum ItemRandomAttributeType : uint8_t {
	ITEM_RND_ATTACK = 1,
	ITEM_RND_ARMOR,
	ITEM_RND_DEF,
	ITEM_RND_SKILL,
	//ITEM_RND_MAGIC,
	ITEM_RND_WEIGHT_REDUCTION,
	ITEM_RND_MANA_REGEN,
	ITEM_RND_HEALTH_REGEN,
	ITEM_RND_MAX_HEALTH,
	ITEM_RND_MAX_MANA,
	ITEM_RND_RESISTANCE,
	ITEM_RND_ATTACK_SPEED,
	ITEM_RND_CRITICAL,
	ITEM_RND_PARRY,
	ITEM_RND_PERSEVERANCE,
	ITEM_RND_BERSERK,
	ITEM_RND_CRUSHING_BLOW,
	ITEM_RND_FAST_HAND,
	ITEM_RND_SHARPSHOOTER,
	ITEM_RND_BLEEDING,
	...
	ITEM_RND_FIRST = ITEM_RND_ATTACK,
	ITEM_RND_LAST = ITEM_RND_SPEED,
};

enum ItemAttributeDefenseType : uint32_t
{
	ITEM_RND_RESIST_PHYSICAL = 0x7000,
	ITEM_RND_RESIST_FIRE,
	ITEM_RND_RESIST_POISON,
	ITEM_RND_RESIST_ENERGY,
	ITEM_RND_RESIST_ICE,
};

constexpr uint32_t UNCOMMON_ITEM_SLOTS = 1;
constexpr uint32_t RARE_ITEM_SLOTS = 2;
constexpr uint32_t EPIC_ITEM_SLOTS = 3;
constexpr uint32_t LEGENDARY_ITEM_SLOTS = 4;
// custom attribute to indicate rarity level of the item. 
// This number is 'r' and 'y' (from rarity)
constexpr uint16_t ITEM_RARITY_ATTRIBUTE = 29305;
...
class ItemAttributes
{
	public:
		...
		void setUniqueId(uint32_t n) {
			setIntAttr(ITEM_ATTRIBUTE_UNIQUEID, n);
		}
		uint32_t getUniqueId() const {
			return static_cast<uint32_t>(getIntAttr(ITEM_ATTRIBUTE_UNIQUEID));
		}
	...
	private:
		...
		const static uint32_t intAttributeTypes = ITEM_ATTRIBUTE_ACTIONID | ITEM_ATTRIBUTE_UNIQUEID | ITEM_ATTRIBUTE_DATE
			...
			| ITEM_ATTRIBUTE_DOORQUESTNUMBER | ITEM_ATTRIBUTE_DOORQUESTVALUE | ITEM_ATTRIBUTE_AUTOOPEN;
	...
	public:
		...
		static std::string customAttributeMapKeyToString(uint16_t key) {
			std::string result;

			result.push_back(static_cast<const char>((key & 0xFF00) >> 8));
			result.push_back(static_cast<const char>((key & 0x00FF) >> 0));

			return result;
		}

		static uint16_t customAttributeMapKeyToUint16(const std::string key) {
			uint16_t result = 0;
...
class Item : virtual public Thing
{
	public:
		...
		void setStoreItem(bool value) {
			if (value) {
				getAttributes()->setCustomAttribute(ITEM_CUSTOM_ATTRIBUTE_STORE, true);
			}
			else {
				getAttributes()->removeCustomAttribute(ITEM_CUSTOM_ATTRIBUTE_STORE);
			}
		}

		bool isStoreItem() const {
			return const_cast<Item*>(this)->getCustomAttribute(ITEM_CUSTOM_ATTRIBUTE_STORE);
		}
		...
		int64_t getItemRarity();
		int64_t getItemTierRarity();
		ItemTierLevel updateItemAttributes(const Player* player);
		int32_t getRarityAttributeRequiredLevel();
		...
		ItemAttributes::CustomAttributeMap getIntCustomAttributeMap() const {
			ItemAttributes::CustomAttributeMap intCustomAttributeMap;

			if (!attributes) {
				return intCustomAttributeMap;
			}

			auto rawCustomAttributeMap = attributes->getCustomAttributeMap();

			if (rawCustomAttributeMap == nullptr) {
				return intCustomAttributeMap;
			}

			for (auto it = rawCustomAttributeMap->begin(); it != rawCustomAttributeMap->end(); it++) {
				std::string customAttributeKey = it->first;
				ItemAttributes::CustomAttribute customAttributeValue = it->second;

				if (customAttributeValue.value.type() == typeid(int64_t)) {
					intCustomAttributeMap.emplace(customAttributeKey, customAttributeValue);
				}
			}
		...
		int64_t getIntCustomAttribute(const Player* player, std::string key);
		bool canPlayerUseRarityAttribute(const Player* player, int tier);
		...
		virtual Attr_ReadValue readAttr2(AttrTypes_t attr, PropStream& propStream);
		bool unserializeAttr(PropStream& propStream, bool uid = true);
		...
		ShootType_t getShootType() const {
    ...
    void setRealUID(uint32_t uid) {
		...
		// This variable is merely used for tiles for monsters such as Fire Elementals
		// So their corpses can drop on tiles with existing fields
		// This cannot be easily hacked because we cannot have "Flags" for adding items in tiles, only for queries
		bool specialCorpseDrop = false;
		...
		void rollRarityLevel(bool forceRarity = false, uint8_t rarity = 1);
		void rollAttributes(ItemTierLevel tierLevel, uint8_t totalAttributes);
		void rollAttributes(ItemTierLevel tierLevel, uint8_t totalAttributes, std::vector<ItemRandomAttributeType> attrList);
		void applyRarity(ItemTierLevel tierLevel = ITEM_TIER_1, ItemRarityLevel rarityLevel = ITEM_COMMON, std::vector<ItemRandomAttributeType> attrList = {});
		void addArmorAttribute();
		void applyFixedBonuses();
