...
int32_t Tile::getHeight() const
{
	...
	if (const TileItemVector* items = getItemList()) {
		for (auto it = items->rbegin(), end = items->rend(); it != end; ++it) {
			const Item* item = (*it);
			const ItemType& type = Item::items.getItemType(item->getID());
			if (type.hasHeight) {
				height++;
			}
		}
	}
	return std::min<int32_t>(4, height);
}
		...
		if (itemIsHangable && hasFlag(TILESTATE_SUPPORTS_HANGABLE)) {
			...
			if (items) {
				...
				for (const Item* tileItem : *items) {
					...
					if (!iiType.blockSolid || hasBitSet(FLAG_IGNOREBLOCKITEM, flags)) {
						continue;
					}
			...
			if (itemType.isMagicField() || itemType.replaceMagicFields) {
				...
				if (items) {
					for (ItemVector::const_iterator it = items->getBeginDownItem(), end = items->getEndDownItem(); it != end; ++it) {
						...
						if (oldField) {
							if (!oldField->canDecay() && !oldField->getDamage() || item->specialCorpseDrop && !oldField->canDecay()) {
								// Case for small fire field, new fields should go on top
								continue;
							} else if (oldField->canDecay() && oldField->isReplaceable()) {
								// replace decaying & replacable magic fields
								removeThing(oldField, 1);
								oldField->setParent(nullptr);
								g_game.ReleaseItem(oldField);
								postRemoveNotification(oldField, nullptr, 0);
								break;
								} else if (!oldField->isReplaceable()) {
