...
GlobalEventMap GlobalEvents::getEventMap(GlobalEvent_t type)
{
	...
	switch (type) {
		...
		case GLOBALEVENT_HOUSE_TRANSFER_ITEMS:
...
bool GlobalEvent::configureEvent(const pugi::xml_node& node)
{
	...
	if ((attr = node.attribute("time"))) {
		...
		} else if (strcasecmp(value, "housetransferitems") == 0) {
			eventType = GLOBALEVENT_HOUSE_TRANSFER_ITEMS;
		} else {
...
std::string GlobalEvent::getScriptEventName() const
{
	switch (eventType) {
		...
		case GLOBALEVENT_HOUSE_TRANSFER_ITEMS: return "onHouseTransferItems";
...
bool GlobalEvent::executeHouseTransferToDepot(Player* player, uint32_t townId)
{
	//onHouseTransferItems(player, town)
	if (!scriptInterface->reserveScriptEnv()) {
		std::cout << "[Error - GlobalEvent::executeHouseTransferToDepot] Call stack overflow" << std::endl;
		return false;
	}
