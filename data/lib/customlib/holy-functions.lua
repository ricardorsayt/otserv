function entregarQuests(pid)
	...
	if player then
		-- As linhas abaixo foram comentadas para desativar a entrega automatica de quests,
		-- achievements e outfits. Os jogadores terao que completar as missoes.
		-- for i = 1, #missions do
...
-- Nova funcao: Monster.isBoosted
function Monster.isBoosted(self)
    -- Garante que 'self' e um monstro valido
    if not self or not self:isMonster() then
        return false
    end

    -- Converte o nome do monstro para minusculas para comparacao
    local monsterName = self:getName():lower()

    -- Verifica se a tabela global 'boostCreature' existe e nao esta vazia
    if boostCreature then
        for _, boosted in ipairs(boostCreature) do
            -- Compara o nome do monstro com o nome do monstro boosted (em minusculas)
            if boosted.name == monsterName then
                return true
            end
        end
    end
