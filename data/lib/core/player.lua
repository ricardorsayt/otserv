...
function Player.setPlayerAura(self, aura)
    local outfit = self:getOutfit()
    outfit.lookAura = aura
    self:setOutfit(outfit)
end

function Player.setPlayerWing(self, wing)
    local outfit = self:getOutfit()
    outfit.lookWing = wing
    self:setOutfit(outfit)
end

function Player.setPlayerShader(self, shader)
    local outfit = self:getOutfit()
    outfit.lookShader = shader
    self:setOutfit(outfit)
end

function Player.setMapShader(self, shaderName)

    self:sendExtendedOpcode(SHADER_MAP_CODE, json.encode({action = "setMap", data = {shader = shaderName}}))
end


function Player.removeMapShader(self)
	self:sendExtendedOpcode(SHADER_MAP_CODE, json.encode({action = "remove", data = {}}))
end