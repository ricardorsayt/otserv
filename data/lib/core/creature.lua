function Creature.getClosestFreePosition(self, position, maxRadius, mustBeReachable, ignorePz)
	...
	for radius = 0, maxRadius do
		...
		for i = 1, total do
			...
			if tile and tile:getCreatureCount() == 0 and not tile:hasProperty(CONST_PROP_IMMOVABLEBLOCKSOLID) and
				(not mustBeReachable or self:getPathTo(checkPosition)) and
				(not ignorePz or tile:hasFlag(TILESTATE_PROTECTIONZONE))
			then
