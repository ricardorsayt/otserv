...
local function handleQuickLoot(player, opcode, buffer)
	...
	if not data then
		print("[DEBUG] Auto-Loot: Failed to decode JSON data.")
		return false
	end
	print("[DEBUG] Auto-Loot: Received request. Action: " .. tostring(data.action))
	...
	elseif action == "search" then
		...
		local monsters = {}
		for _, mType in ipairs(MonsterType.list()) do
			if mType:getName():lower():find(search, 1, true) then
				table.insert(monsters, mType:getName())
			end
		end
		local responseData = {
