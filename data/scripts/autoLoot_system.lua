...
local jsonOk, json = pcall(require, "cjson") -- usa cjson se disponível
if not jsonOk then
  -- fallback simples (opcional): tente "json" / "dkjson" se existirem no seu distro
...
local config = {
  ...
  GOLD_POUCH = 26377,
  blockedIds = {2393}, -- bloquear adicionar itens específicos na lista
}
...
-- =========================
-- Coleta pós-kill (v1) + SEMPRE moedas
-- =========================
function AutoLootList.getLootItem(self, playerCid, position)
  ...
  -- mesmo que lista esteja vazia, ainda vale a pena tentar coletar moedas
  if itemCount == 0 and not true then return false end
...
function system_autoloot_onKill.onKill(creature, target)
  ...
  -- passa o CID (runtime id); dentro nós pegamos o GUID para a lista
...
-- =========================
-- Funções add/remove rápidas
-- =========================
local function addQuickItem(playerGuid, itemId, itemName)
  local player = Player(playerGuid) -- aceita GUID no nome, mas vamos pegar objeto via cid quando necessário
...
moveItemEvent.onMoveItem = function(player, item, count, fromPos, toPos, fromCyl, toCyl)
  ...
  if toCyl:getId() == config.GOLD_POUCH then
    -- permite mover moedas ou itens que já estão na lista do jogador
