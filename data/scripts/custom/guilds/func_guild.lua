-- Função adicionada para obter todas as guildas do banco de dados
function getAllGuilds()
    local resultId = db.storeQuery("SELECT `id` FROM `guilds`")
...
function getTopGuilds(force)
  if force or lastTopGuildsFetch + GUILDS_CFG.TOP_GUILDS_CACHE < os.time() then
    ...
    local tempTopGuilds = {}
    -- CORREÇÃO: Usando a nova função para obter todas as guildas
...
if not Guild.getPacifism then
  function Guild:getPacifism()
    local res = db.storeQuery(string.format("SELECT pacifism FROM guilds WHERE id=%d", self:getId()))
    if res ~= false then
      local v = result.getNumber(res, "pacifism")
      result.free(res)
      return v
    end
    return 0
...
if not Guild.getPacifismStatus then
  function Guild:getPacifismStatus()
    local res = db.storeQuery(string.format("SELECT pacifism_status FROM guilds WHERE id=%d", self:getId()))
    if res ~= false then
      local v = result.getNumber(res, "pacifism_status")
      result.free(res)
      return v
    end
    return 0
...
-- Polyfills seguros para métodos de Guild usados em getTopGuilds

if not Guild.getLevel then
  function Guild:getLevel()
    local res = db.storeQuery(string.format("SELECT level FROM guilds WHERE id=%d LIMIT 1", self:getId()))
    if res ~= false then
      local v = result.getNumber(res, "level")
      result.free(res)
      return v or 1
    end
    return 1
...
if not Guild.getWarsWon then
  function Guild:getWarsWon()
    local res = db.storeQuery(string.format("SELECT wars_won FROM guilds WHERE id=%d LIMIT 1", self:getId()))
    if res ~= false then
      local v = result.getNumber(res, "wars_won")
      result.free(res)
      return v or 0
    end
    return 0
...
if not Guild.getWarsLost then
  function Guild:getWarsLost()
    local res = db.storeQuery(string.format("SELECT wars_lost FROM guilds WHERE id=%d LIMIT 1", self:getId()))
    if res ~= false then
      local v = result.getNumber(res, "wars_lost")
      result.free(res)
      return v or 0
    end
    return 0
...
-- Se sua base usa guild_membership (player_id, guild_id, rank_id)
if not Guild.getMemberCount then
  function Guild:getMemberCount()
    local res = db.storeQuery(string.format("SELECT COUNT(*) AS c FROM guild_membership WHERE guild_id=%d", self:getId()))
    if res ~= false then
      local c = result.getNumber(res, "c")
      result.free(res)
      return c or 0
    end
    return 0
...
-- Soma níveis dos membros (players.level) da guild
if not Guild.getCombinedLevels then
  function Guild:getCombinedLevels()
    local res = db.storeQuery(string.format([[
      SELECT COALESCE(SUM(p.level),0) AS s
      FROM guild_membership m
      JOIN players p ON p.id = m.player_id
      WHERE m.guild_id = %d
    ]], self:getId()))
    if res ~= false then
      local s = result.getNumber(res, "s")
      result.free(res)
      return s or 0
    end
    return 0
