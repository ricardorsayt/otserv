local marketing_area = {
	{["init"] = {x = 33120, y = 32837, z = 8}, ["end"] = {x = 33132, y = 32849, z = 8}}, -- Ankrahmun
	...
	if opcode == 96 then
					...
					if item:getId() == Game.getItemTypeByClientId(buffer_decode.data.id):getId() then
						if item:getCharges() > 1 and item:getCharges() == buffer_decode.data.quant or item:getCount() == buffer_decode.data.quant then
							...
							if inMarketArea then
								...
								if not item:hasAttribute("uid") then
									uid = 65536
								...
								Game.addMarketingOffer(player:getName(), item:getAttribute("uid"), item, buffer_decode.data.price, buffer_decode.data.rarity)
		...
		elseif buffer_decode.action == "Start" then
			...
			local function formatMarketDescription(description)
				...
				local market = Game.createMonster("Market", player:getPosition(), true)
				if market then
					...
					print("Nao foi possivel criar o Market.")
				end
			else
