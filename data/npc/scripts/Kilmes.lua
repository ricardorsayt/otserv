local keywordHandler = KeywordHandler:new()
local npcHandler = NpcHandler:new(keywordHandler)
NpcSystem.parseParameters(npcHandler)

-- OTServ event handling functions start
function onCreatureAppear(cid)
    npcHandler:onCreatureAppear(cid)
end

function onCreatureDisappear(cid)
    npcHandler:onCreatureDisappear(cid)
end

function onCreatureSay(cid, type, msg)
    npcHandler:onCreatureSay(cid, type, msg)
end

function onThink()
    npcHandler:onThink()
end

function onPlayerEndTrade(cid)
    npcHandler:onPlayerEndTrade(cid)
end

function onPlayerCloseChannel(cid)
    npcHandler:onPlayerCloseChannel(cid)
end
-- OTServ event handling functions end

local shopModule = ShopModule:new()
npcHandler:addModule(shopModule)

-------------------------------he sells--------------------------------------

-- NPC VENDE ARMOR
shopModule:addBuyableItem({'Blue Robe'}, 2656, 30000, 'Blue Robe')
shopModule:addBuyableItem({'Brass Armor'}, 2465, 450, 'Brass Armor')
shopModule:addBuyableItem({'Calopteryx Cape'}, 15489, 45000, 'Calopteryx Cape')
shopModule:addBuyableItem({'Chain Armor'}, 2464, 210, 'Chain Armor')
shopModule:addBuyableItem({'Coat'}, 2651, 3, 'Coat')
shopModule:addBuyableItem({'Crown Armor'}, 2487, 36000, 'Crown Armor')
shopModule:addBuyableItem({'Crystalline Armor'}, 8878, 48000, 'Crystalline Armor')
shopModule:addBuyableItem({'Dark Armor'}, 2489, 1200, 'Dark Armor')
shopModule:addBuyableItem({'Depth Lorica'}, 15407, 9900500, 'Depth Lorica')
shopModule:addBuyableItem({'Divine Plate'}, 8885, 5165000, 'Divine Plate')
shopModule:addBuyableItem({'Doublet'}, 2485, 9, 'Doublet')
shopModule:addBuyableItem({'Dragon Scale Mail'}, 2492, 120000, 'Dragon Scale Mail')
shopModule:addBuyableItem({'Dwarven Armor'}, 2503, 2900000, 'Dwarven Armor')
shopModule:addBuyableItem({'Elite Draken Mail'}, 12607, 1500000, 'Elite Draken Mail')
shopModule:addBuyableItem({'Glacier Kilt'}, 7896, 33000, 'Glacier Kilt')
shopModule:addBuyableItem({'Glacier Robe'}, 7897, 33000, 'Glacier Robe')
shopModule:addBuyableItem({'Golden Armor'}, 2466, 60000, 'Golden Armor')
shopModule:addBuyableItem({'Hibiscus Dress'}, 8873, 9000, 'Hibiscus Dress')
shopModule:addBuyableItem({'Jacket'}, 2650, 3, 'Jacket')
shopModule:addBuyableItem({'Knight Armor'}, 2476, 15000, 'Knight Armor')
shopModule:addBuyableItem({'Lavos Armor'}, 8877, 248000, 'Lavos Armor')
shopModule:addBuyableItem({'Leather Armor'}, 2467, 36, 'Leather Armor')
shopModule:addBuyableItem({'Leopard Armor'}, 3968, 3000, 'Leopard Armor')
shopModule:addBuyableItem({'Lightning Robe'}, 7898, 33000, 'Lightning Robe')
shopModule:addBuyableItem({'Magic Plate Armor'}, 2472, 270000, 'Magic Plate Armor')
shopModule:addBuyableItem({'Magma Coat'}, 7899, 33000, 'Magma Coat')
shopModule:addBuyableItem({'Noble Armor'}, 2486, 2700, 'Noble Armor')
shopModule:addBuyableItem({'Ornate Chestplate'}, 15406, 18000000, 'Ornate Chestplate')
shopModule:addBuyableItem({'Paladin Armor'}, 8891, 75000, 'Paladin Armor')
shopModule:addBuyableItem({'Plate Armor'}, 2463, 1200, 'Plate Armor')
shopModule:addBuyableItem({'Scale Armor'}, 2483, 225, 'Scale Armor')
shopModule:addBuyableItem({'Skullcracker Armor'}, 8889, 104000, 'Skullcracker Armor')
shopModule:addBuyableItem({'Studded Armor'}, 2484, 75, 'Studded Armor')
shopModule:addBuyableItem({'Swamplair Armor'}, 8880, 248000, 'Swamplair Armor')
shopModule:addBuyableItem({'Terra Mantle'}, 7884, 33000, 'Terra Mantle')
shopModule:addBuyableItem({'Zaoan Armor'}, 11301, 72000, 'Zaoan Armor')

-- NPC VENDE LEGS
shopModule:addBuyableItem({'Brass Legs'}, 2478, 147, 'Brass Legs')
shopModule:addBuyableItem({'Chain Legs'}, 2648, 75, 'Chain Legs')
shopModule:addBuyableItem({'Crown Legs'}, 2488, 36000, 'Crown Legs')
shopModule:addBuyableItem({'Depth Ocrea'}, 15409, 4800000, 'Depth Ocrea')
shopModule:addBuyableItem({'Golden Legs'}, 2470, 390000, 'Golden Legs')
shopModule:addBuyableItem({'Grasshopper Legs'}, 15490, 445000, 'Grasshopper Legs')
shopModule:addBuyableItem({'Knight Legs'}, 2477, 15000, 'Knight Legs')
shopModule:addBuyableItem({'Leaf Legs'}, 9928, 1500, 'Leaf Legs')
shopModule:addBuyableItem({'Leather Legs'}, 2649, 27, 'Leather Legs')
shopModule:addBuyableItem({'Lightning Legs'}, 7895, 33000, 'Lightning Legs')
shopModule:addBuyableItem({'Magma Legs'}, 7894, 33000, 'Magma Legs')
shopModule:addBuyableItem({'Ornate Legs'}, 15412, 12000000, 'Ornate Legs')
shopModule:addBuyableItem({'Plate Legs'}, 2647, 345, 'Plate Legs')
shopModule:addBuyableItem({'Studded Legs'}, 2468, 45, 'Studded Legs')
shopModule:addBuyableItem({'Terra Legs'}, 7885, 33000, 'Terra Legs')
shopModule:addBuyableItem({'Zaoan Legs'}, 11304, 72000, 'Zaoan Legs')

-- NPC VENDE SHIELD
shopModule:addBuyableItem({'Ancient Shield'}, 2535, 2700, 'Ancient Shield')
shopModule:addBuyableItem({'Battle Shield'}, 2513, 285, 'Battle Shield')
shopModule:addBuyableItem({'Black Shield'}, 2529, 2400, 'Black Shield')
shopModule:addBuyableItem({'Bone Shield'}, 2541, 240, 'Bone Shield')
shopModule:addBuyableItem({'Bonelord Shield'}, 2518, 23600, 'Bonelord Shield')
shopModule:addBuyableItem({'Brass Shield'}, 2511, 75, 'Brass Shield')
shopModule:addBuyableItem({'Carapace Shield'}, 15491, 196000, 'Carapace Shield')
shopModule:addBuyableItem({'Copper Shield'}, 2530, 150, 'Copper Shield')
shopModule:addBuyableItem({'Crown Shield'}, 2519, 24000, 'Crown Shield')
shopModule:addBuyableItem({'Dark Shield'}, 2521, 1200, 'Dark Shield')
shopModule:addBuyableItem({'Demon Shield'}, 2520, 120000, 'Demon Shield')
shopModule:addBuyableItem({'Depth Scutum'}, 15411, 10800000, 'Depth Scutum')
shopModule:addBuyableItem({'Dragon Shield'}, 2516, 12000, 'Dragon Shield')
shopModule:addBuyableItem({'Dwarven Shield'}, 2525, 300, 'Dwarven Shield')
shopModule:addBuyableItem({'Griffin Shield'}, 2533, 39000, 'Griffin Shield')
shopModule:addBuyableItem({'Guardian Shield'}, 2515, 6000, 'Guardian Shield')
shopModule:addBuyableItem({'Mastermind Shield'}, 2514, 350000, 'Mastermind Shield')
shopModule:addBuyableItem({'Medusa Shield'}, 2536, 37000, 'Medusa Shield')
shopModule:addBuyableItem({'Norse Shield'}, 7460, 4500, 'Norse Shield')
shopModule:addBuyableItem({'Ornate Shield'}, 15413, 12600000, 'Ornate Shield')
shopModule:addBuyableItem({'Plate Shield'}, 2510, 135, 'Plate Shield')
shopModule:addBuyableItem({'Rift Shield'}, 25382, 5500000, 'Rift Shield')
shopModule:addBuyableItem({'Scarab Shield'}, 2540, 6000, 'Scarab Shield')
shopModule:addBuyableItem({'Steel Shield'}, 2509, 240, 'Steel Shield')
shopModule:addBuyableItem({'Studded Shield'}, 2526, 48, 'Studded Shield')
shopModule:addBuyableItem({'Tortoise Shield'}, 6131, 450, 'Tortoise Shield')
shopModule:addBuyableItem({'Tower Shield'}, 2528, 24000, 'Tower Shield')
shopModule:addBuyableItem({'Vampire Shield'}, 2534, 65000, 'Vampire Shield')
shopModule:addBuyableItem({'Viking Shield'}, 2531, 255, 'Viking Shield')
shopModule:addBuyableItem({'Warrior\'s Shield'}, 15453, 27000, 'Warrior\'s Shield')
shopModule:addBuyableItem({'Wooden Shield'}, 2512, 15, 'Wooden Shield')

-- NPC VENDE HELMET
shopModule:addBuyableItem({'Bandana'}, 5917, 450, 'Bandana')
shopModule:addBuyableItem({'Bonelord Helmet'}, 3972, 122500, 'Bonelord Helmet')
shopModule:addBuyableItem({'Brass Helmet'}, 2460, 90, 'Brass Helmet')
shopModule:addBuyableItem({'Chain Helmet'}, 2458, 51, 'Chain Helmet')
shopModule:addBuyableItem({'Cobra Crown'}, 12630, 2150000, 'Cobra Crown')
shopModule:addBuyableItem({'Crown Helmet'}, 2491, 7500, 'Crown Helmet')
shopModule:addBuyableItem({'Crusader Helmet'}, 2497, 18000, 'Crusader Helmet')
shopModule:addBuyableItem({'Dark Helmet'}, 2490, 750, 'Dark Helmet')
shopModule:addBuyableItem({'Depth Galea'}, 15408, 10105000, 'Depth Galea')
shopModule:addBuyableItem({'Devil Helmet'}, 2462, 3000, 'Devil Helmet')
shopModule:addBuyableItem({'Glacier Mask'}, 7902, 7500, 'Glacier Mask')
shopModule:addBuyableItem({'Helmet of the Lost'}, 20132, 6000, 'Helmet of the Lost')
shopModule:addBuyableItem({'Iron Helmet'}, 2459, 450, 'Iron Helmet')
shopModule:addBuyableItem({'Krimhorn Helmet'}, 7461, 600, 'Krimhorn Helmet')
shopModule:addBuyableItem({'Leather Helmet'}, 2461, 12, 'Leather Helmet')
shopModule:addBuyableItem({'Legion Helmet'}, 2480, 66, 'Legion Helmet')
shopModule:addBuyableItem({'Lightning Headband'}, 7901, 7500, 'Lightning Headband')
shopModule:addBuyableItem({'Magma Monocle'}, 7900, 7500, 'Magma Monocle')
shopModule:addBuyableItem({'Mystic Turban'}, 2663, 450, 'Mystic Turban')
shopModule:addBuyableItem({'Pirate Hat'}, 6096, 3000, 'Pirate Hat')
shopModule:addBuyableItem({'Ragnir Helmet'}, 7462, 1200, 'Ragnir Helmet')
shopModule:addBuyableItem({'Royal Helmet'}, 2498, 90000, 'Royal Helmet')
shopModule:addBuyableItem({'Skull Helmet'}, 5741, 120000, 'Skull Helmet')
shopModule:addBuyableItem({'Soldier Helmet'}, 2481, 48, 'Soldier Helmet')
shopModule:addBuyableItem({'Steel Helmet'}, 2457, 879, 'Steel Helmet')
shopModule:addBuyableItem({'Strange Helmet'}, 2479, 1500, 'Strange Helmet')
shopModule:addBuyableItem({'Studded Helmet'}, 2482, 60, 'Studded Helmet')
shopModule:addBuyableItem({'Terra Hood'}, 7903, 7500, 'Terra Hood')
shopModule:addBuyableItem({'Viking Helmet'}, 2473, 198, 'Viking Helmet')
shopModule:addBuyableItem({'Warrior Helmet'}, 2475, 15000, 'Warrior Helmet')
shopModule:addBuyableItem({'Witch Hat'}, 10570, 15000, 'Witch Hat')
shopModule:addBuyableItem({'Zaoan Helmet'}, 11302, 935000, 'Zaoan Helmet')

-- NPC VENDE BOOTS
shopModule:addBuyableItem({'Boots of Haste'}, 2195, 90000, 'Boots of Haste')
shopModule:addBuyableItem({'Coconut Shoes'}, 9931, 1500, 'Coconut Shoes')
shopModule:addBuyableItem({'Crocodile Boots'}, 3982, 3000, 'Crocodile Boots')
shopModule:addBuyableItem({'Draken Boots'}, 12646, 120000, 'Draken Boots')
shopModule:addBuyableItem({'Fur Boots'}, 7457, 6000, 'Fur Boots')
shopModule:addBuyableItem({'Glacier Shoes'}, 7892, 7500, 'Glacier Shoes')
shopModule:addBuyableItem({'Guardian Boots'}, 11240, 105000, 'Guardian Boots')
shopModule:addBuyableItem({'Leather Boots'}, 2643, 6, 'Leather Boots')
shopModule:addBuyableItem({'Lightning Boots'}, 7893, 7500, 'Lightning Boots')
shopModule:addBuyableItem({'Magma Boots'}, 7891, 7500, 'Magma Boots')
shopModule:addBuyableItem({'Oriental Shoes'}, 24637, 45000, 'Oriental Shoes')
shopModule:addBuyableItem({'Patched Boots'}, 2641, 6000, 'Patched Boots')
shopModule:addBuyableItem({'Pirate Boots'}, 5462, 9000, 'Pirate Boots')
shopModule:addBuyableItem({'Steel Boots'}, 2645, 90000, 'Steel Boots')
shopModule:addBuyableItem({'Terra Boots'}, 7886, 7500, 'Terra Boots')
shopModule:addBuyableItem({'Zaoan Shoes'}, 11303, 15000, 'Zaoan Shoes')


--------------------------------he buys--------------------------------------

-- NPC COMPRA ARMOR
shopModule:addSellableItem({'Albino Plate'}, 21692, 1500, 'Albino Plate')
shopModule:addSellableItem({'Blue Robe'}, 2656, 10000, 'Blue Robe')
shopModule:addSellableItem({'Brass Armor'}, 2465, 150, 'Brass Armor')
shopModule:addSellableItem({'Calopteryx Cape'}, 15489, 15000, 'Calopteryx Cape')
shopModule:addSellableItem({'Chain Armor'}, 2464, 70, 'Chain Armor')
shopModule:addSellableItem({'Coat'}, 2651, 1, 'Coat')
shopModule:addSellableItem({'Crown Armor'}, 2487, 12000, 'Crown Armor')
shopModule:addSellableItem({'Crystalline Armor'}, 8878, 16000, 'Crystalline Armor')
shopModule:addSellableItem({'Dark Armor'}, 2489, 400, 'Dark Armor')
shopModule:addSellableItem({'Depth Lorica'}, 15407, 30000, 'Depth Lorica')
shopModule:addSellableItem({'Divine Plate'}, 8885, 55000, 'Divine Plate')
shopModule:addSellableItem({'Doublet'}, 2485, 3, 'Doublet')
shopModule:addSellableItem({'Dragon Scale Mail'}, 2492, 40000, 'Dragon Scale Mail')
shopModule:addSellableItem({'Dwarven Armor'}, 2503, 30000, 'Dwarven Armor')
shopModule:addSellableItem({'Elite Draken Mail'}, 12607, 50000, 'Elite Draken Mail')
shopModule:addSellableItem({'Glacier Kilt'}, 7896, 11000, 'Glacier Kilt')
shopModule:addSellableItem({'Glacier Robe'}, 7897, 11000, 'Glacier Robe')
shopModule:addSellableItem({'Golden Armor'}, 2466, 20000, 'Golden Armor')
shopModule:addSellableItem({'Hibiscus Dress'}, 8873, 3000, 'Hibiscus Dress')
shopModule:addSellableItem({'Jacket'}, 2650, 1, 'Jacket')
shopModule:addSellableItem({'Knight Armor'}, 2476, 5000, 'Knight Armor')
shopModule:addSellableItem({'Lavos Armor'}, 8877, 16000, 'Lavos Armor')
shopModule:addSellableItem({'Leather Armor'}, 2467, 12, 'Leather Armor')
shopModule:addSellableItem({'Leopard Armor'}, 3968, 1000, 'Leopard Armor')
shopModule:addSellableItem({'Lightning Robe'}, 7898, 11000, 'Lightning Robe')
shopModule:addSellableItem({'Magic Plate Armor'}, 2472, 90000, 'Magic Plate Armor')
shopModule:addSellableItem({'Magma Coat'}, 7899, 11000, 'Magma Coat')
shopModule:addSellableItem({'Noble Armor'}, 2486, 900, 'Noble Armor')
shopModule:addSellableItem({'Ornate Chestplate'}, 15406, 60000, 'Ornate Chestplate')
shopModule:addSellableItem({'Paladin Armor'}, 8891, 15000, 'Paladin Armor')
shopModule:addSellableItem({'Pirate Knee Breeches'}, 5918, 200, 'Pirate Knee Breeches')
shopModule:addSellableItem({'Pirate Shirt'}, 6095, 500, 'Pirate Shirt')
shopModule:addSellableItem({'Plate Armor'}, 2463, 400, 'Plate Armor')
shopModule:addSellableItem({'Scale Armor'}, 2483, 75, 'Scale Armor')
shopModule:addSellableItem({'Skullcracker Armor'}, 8889, 18000, 'Skullcracker Armor')
shopModule:addSellableItem({'Studded Armor'}, 2484, 25, 'Studded Armor')
shopModule:addSellableItem({'Swamplair Armor'}, 8880, 16000, 'Swamplair Armor')
shopModule:addSellableItem({'Terra Mantle'}, 7884, 11000, 'Terra Mantle')
shopModule:addSellableItem({'Zaoan Armor'}, 11301, 14000, 'Zaoan Armor')

-- NPC COMPRA LEGS
shopModule:addSellableItem({'Brass Legs'}, 2478, 49, 'Brass Legs')
shopModule:addSellableItem({'Chain Legs'}, 2648, 25, 'Chain Legs')
shopModule:addSellableItem({'Crown Legs'}, 2488, 12000, 'Crown Legs')
shopModule:addSellableItem({'Depth Ocrea'}, 15409, 16000, 'Depth Ocrea')
shopModule:addSellableItem({'Golden Legs'}, 2470, 30000, 'Golden Legs')
shopModule:addSellableItem({'Grasshopper Legs'}, 15490, 15000, 'Grasshopper Legs')
shopModule:addSellableItem({'Knight Legs'}, 2477, 5000, 'Knight Legs')
shopModule:addSellableItem({'Leaf Legs'}, 9928, 500, 'Leaf Legs')
shopModule:addSellableItem({'Leather Legs'}, 2649, 9, 'Leather Legs')
shopModule:addSellableItem({'Lightning Legs'}, 7895, 11000, 'Lightning Legs')
shopModule:addSellableItem({'Magma Legs'}, 7894, 11000, 'Magma Legs')
shopModule:addSellableItem({'Ornate Legs'}, 15412, 40000, 'Ornate Legs')
shopModule:addSellableItem({'Plate Legs'}, 2647, 115, 'Plate Legs')
shopModule:addSellableItem({'Studded Legs'}, 2468, 15, 'Studded Legs')
shopModule:addSellableItem({'Terra Legs'}, 7885, 11000, 'Terra Legs')
shopModule:addSellableItem({'Zaoan Legs'}, 11304, 14000, 'Zaoan Legs')

-- NPC COMPRA SHIELD
shopModule:addSellableItem({'Ancient Shield'}, 2535, 900, 'Ancient Shield')
shopModule:addSellableItem({'Battle Shield'}, 2513, 95, 'Battle Shield')
shopModule:addSellableItem({'Black Shield'}, 2529, 800, 'Black Shield')
shopModule:addSellableItem({'Bone Shield'}, 2541, 80, 'Bone Shield')
shopModule:addSellableItem({'Bonelord Shield'}, 2518, 1200, 'Bonelord Shield')
shopModule:addSellableItem({'Brass Shield'}, 2511, 25, 'Brass Shield')
shopModule:addSellableItem({'Carapace Shield'}, 15491, 32000, 'Carapace Shield')
shopModule:addSellableItem({'Castle Shield'}, 2535, 5000, 'Castle Shield')
shopModule:addSellableItem({'Copper Shield'}, 2530, 50, 'Copper Shield')
shopModule:addSellableItem({'Crown Shield'}, 2519, 8000, 'Crown Shield')
shopModule:addSellableItem({'Dark Shield'}, 2521, 400, 'Dark Shield')
shopModule:addSellableItem({'Demon Shield'}, 2520, 30000, 'Demon Shield')
shopModule:addSellableItem({'Depth Scutum'}, 15411, 36000, 'Depth Scutum')
shopModule:addSellableItem({'Dragon Shield'}, 2516, 4000, 'Dragon Shield')
shopModule:addSellableItem({'Dwarven Shield'}, 2525, 100, 'Dwarven Shield')
shopModule:addSellableItem({'Griffin Shield'}, 2533, 3000, 'Griffin Shield')
shopModule:addSellableItem({'Guardian Shield'}, 2515, 2000, 'Guardian Shield')
shopModule:addSellableItem({'Mastermind Shield'}, 2514, 50000, 'Mastermind Shield')
shopModule:addSellableItem({'Medusa Shield'}, 2536, 9000, 'Medusa Shield')
shopModule:addSellableItem({'Norse Shield'}, 7460, 1500, 'Norse Shield')
shopModule:addSellableItem({'Ornate Shield'}, 15413, 42000, 'Ornate Shield')
shopModule:addSellableItem({'Phoenix Shield'}, 2539, 16000, 'Phoenix Shield')
shopModule:addSellableItem({'Plate Shield'}, 2510, 45, 'Plate Shield')
shopModule:addSellableItem({'Rift Shield'}, 25382, 50000, 'Rift Shield')
shopModule:addSellableItem({'Scarab Shield'}, 2540, 2000, 'Scarab Shield')
shopModule:addSellableItem({'Steel Shield'}, 2509, 80, 'Steel Shield')
shopModule:addSellableItem({'Studded Shield'}, 2526, 16, 'Studded Shield')
shopModule:addSellableItem({'Tempest Shield'}, 2542, 35000, 'Tempest Shield')
shopModule:addSellableItem({'Tortoise Shield'}, 6131, 150, 'Tortoise Shield')
shopModule:addSellableItem({'Tower Shield'}, 2528, 8000, 'Tower Shield')
shopModule:addSellableItem({'Vampire Shield'}, 2534, 15000, 'Vampire Shield')
shopModule:addSellableItem({'Viking Shield'}, 2531, 85, 'Viking Shield')
shopModule:addSellableItem({'Warrior\'s Shield'}, 15453, 9000, 'Warrior\'s Shield')
shopModule:addSellableItem({'Wooden Shield'}, 2512, 5, 'Wooden Shield')

-- NPC COMPRA HELMET
shopModule:addSellableItem({'Bandana'}, 5917, 150, 'Bandana')
shopModule:addSellableItem({'Bonelord Helmet'}, 3972, 7500, 'Bonelord Helmet')
shopModule:addSellableItem({'Brass Helmet'}, 2460, 30, 'Brass Helmet')
shopModule:addSellableItem({'Chain Helmet'}, 2458, 17, 'Chain Helmet')
shopModule:addSellableItem({'Cobra Crown'}, 12630, 50000, 'Cobra Crown')
shopModule:addSellableItem({'Crown Helmet'}, 2491, 2500, 'Crown Helmet')
shopModule:addSellableItem({'Crusader Helmet'}, 2497, 6000, 'Crusader Helmet')
shopModule:addSellableItem({'Dark Helmet'}, 2490, 250, 'Dark Helmet')
shopModule:addSellableItem({'Depth Galea'}, 15408, 35000, 'Depth Galea')
shopModule:addSellableItem({'Devil Helmet'}, 2462, 1000, 'Devil Helmet')
shopModule:addSellableItem({'Glacier Mask'}, 7902, 2500, 'Glacier Mask')
shopModule:addSellableItem({'Helmet of the Lost'}, 20132, 2000, 'Helmet of the Lost')
shopModule:addSellableItem({'Iron Helmet'}, 2459, 150, 'Iron Helmet')
shopModule:addSellableItem({'Krimhorn Helmet'}, 7461, 200, 'Krimhorn Helmet')
shopModule:addSellableItem({'Leather Helmet'}, 2461, 4, 'Leather Helmet')
shopModule:addSellableItem({'Legion Helmet'}, 2480, 22, 'Legion Helmet')
shopModule:addSellableItem({'Lightning Headband'}, 7901, 2500, 'Lightning Headband')
shopModule:addSellableItem({'Magma Monocle'}, 7900, 2500, 'Magma Monocle')
shopModule:addSellableItem({'Mystic Turban'}, 2663, 150, 'Mystic Turban')
shopModule:addSellableItem({'Pirate Hat'}, 6096, 1000, 'Pirate Hat')
shopModule:addSellableItem({'Ragnir Helmet'}, 7462, 400, 'Ragnir Helmet')
shopModule:addSellableItem({'Royal Helmet'}, 2498, 30000, 'Royal Helmet')
shopModule:addSellableItem({'Skull Helmet'}, 5741, 40000, 'Skull Helmet')
shopModule:addSellableItem({'Soldier Helmet'}, 2481, 16, 'Soldier Helmet')
shopModule:addSellableItem({'Steel Helmet'}, 2457, 293, 'Steel Helmet')
shopModule:addSellableItem({'Strange Helmet'}, 2479, 500, 'Strange Helmet')
shopModule:addSellableItem({'Studded Helmet'}, 2482, 20, 'Studded Helmet')
shopModule:addSellableItem({'Terra Hood'}, 7903, 2500, 'Terra Hood')
shopModule:addSellableItem({'Viking Helmet'}, 2473, 66, 'Viking Helmet')
shopModule:addSellableItem({'Warrior Helmet'}, 2475, 5000, 'Warrior Helmet')
shopModule:addSellableItem({'Witch Hat'}, 10570, 5000, 'Witch Hat')
shopModule:addSellableItem({'Zaoan Helmet'}, 11302, 45000, 'Zaoan Helmet')

-- NPC COMPRA BOOTS
shopModule:addSellableItem({'Boots of Haste'}, 2195, 30000, 'Boots of Haste')
shopModule:addSellableItem({'Coconut Shoes'}, 9931, 500, 'Coconut Shoes')
shopModule:addSellableItem({'Crocodile Boots'}, 3982, 1000, 'Crocodile Boots')
shopModule:addSellableItem({'Draken Boots'}, 12646, 40000, 'Draken Boots')
shopModule:addSellableItem({'Fur Boots'}, 7457, 2000, 'Fur Boots')
shopModule:addSellableItem({'Glacier Shoes'}, 7892, 2500, 'Glacier Shoes')
shopModule:addSellableItem({'Guardian Boots'}, 11240, 35000, 'Guardian Boots')
shopModule:addSellableItem({'Leather Boots'}, 2643, 2, 'Leather Boots')
shopModule:addSellableItem({'Lightning Boots'}, 7893, 2500, 'Lightning Boots')
shopModule:addSellableItem({'Magma Boots'}, 7891, 2500, 'Magma Boots')
shopModule:addSellableItem({'Oriental Shoes'}, 24637, 15000, 'Oriental Shoes')
shopModule:addSellableItem({'Patched Boots'}, 2641, 2000, 'Patched Boots')
shopModule:addSellableItem({'Pirate Boots'}, 5462, 3000, 'Pirate Boots')
shopModule:addSellableItem({'Steel Boots'}, 2645, 30000, 'Steel Boots')
shopModule:addSellableItem({'Terra Boots'}, 7886, 2500, 'Terra Boots')
shopModule:addSellableItem({'Zaoan Shoes'}, 11303, 5000, 'Zaoan Shoes')


npcHandler:addModule(FocusModule:new())